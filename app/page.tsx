...
export default function YouTubeSearchPage() {
  ...
  const formatDate = (dateStr: string): string => {
    ...
    return published.toLocaleDateString('ko-KR', { year: 'numeric', month: 'short', day: 'numeric' });
  };
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/youtube/search?q=${encodeURIComponent(searchQuery)}&regionCode=${regionCode}&maxResults=${maxResults}&dateRange=${dateRange}&videoLength=${videoLength}`
      );
      ...
      <header className="sticky top-0 z-50 bg-neutral-950/80 backdrop-blur-md border-b border-neutral-800 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-red-500 text-2xl font-bold tracking-tighter flex items-center gap-1">
            <span className="inline-block px-1.5 py-0.5 bg-red-500 text-white rounded-md text-sm">▶</span>
            TubeSearch
          </span>
        </div>
        <div className="text-sm text-neutral-400 font-medium">Powered by YouTube API</div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-neutral-400 text-base md:text-lg">
            국가별 트렌드를 반영하여 원하는 영상 데이터를 표 형태로 정갈하게 한눈에 비교해보세요.
          </p>
        </div>
        ...
        <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-2xl mb-12">
          <form onSubmit={handleSearch} className="space-y-6">
            ...
            <div className="relative flex items-center">
              <div className="absolute left-4 text-neutral-500 pointer-events-none text-lg">🔍</div>
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="검색어를 입력하세요 (예: Next.js 개발)..."
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 pt-5 border-t border-neutral-800">
              ...
              <div className="bg-neutral-950/30 p-4 rounded-xl border border-neutral-800/40">
                <label className="block text-xs font-bold uppercase tracking-wider text-neutral-400 mb-3">📅 조회 기간</label>
                <div className="flex flex-wrap gap-1.5">
                  {[{ label: '일주일', value: '1w' }, { label: '1개월', value: '1m' }, { label: '3개월', value: '3m' }, { label: '6개월', value: '6m' }, { label: '1년', value: '1y' }].map((item) => (
                    <button 
                      key={item.value} 
                      type="button" 
                      onClick={() => setDateRange(item.value)} 
                      className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-all ${dateRange === item.value ? 'bg-neutral-100 text-neutral-950 border-neutral-100 shadow-lg' : 'bg-neutral-950 text-neutral-400 border-neutral-800 hover:border-neutral-700 hover:text-neutral-200'}`}
                    >
                      {item.label}
                    </button>
                  ))}
              ...
              <div className="bg-neutral-950/30 p-4 rounded-xl border border-neutral-800/40">
                <label className="block text-xs font-bold uppercase tracking-wider text-neutral-400 mb-3">🌐 검색 대상 국가</label>
                <div className="relative flex items-center">
                  <select 
                    value={regionCode} 
                  ...
                  >
                    {countries.map((country) => (
                      <option key={country.code} value={country.code} className="bg-neutral-900 text-neutral-200">{country.name}</option>
                    ))}
                  </select>
              ...
              {/* 3. 영상 길이 필터 (2행 1열) */}
              <div className="bg-neutral-950/30 p-4 rounded-xl border border-neutral-800/40">
                <label className="block text-xs font-bold uppercase tracking-wider text-neutral-400 mb-3">🎬 영상 길이</label>
                <div className="flex gap-1.5">
                  {[{ label: '전체', value: 'any' }, { label: '숏폼', value: 'short' }, { label: '롱폼', value: 'long' }].map((item) => (
                    <button 
                      key={item.value} 
                      type="button" 
              ...
              {/* 4. 검색 결과 개수 필터 (2행 2열) */}
              <div className="bg-neutral-950/30 p-4 rounded-xl border border-neutral-800/40">
                <label className="block text-xs font-bold uppercase tracking-wider text-neutral-400 mb-3">🔢 조회 개수 (최대)</label>
                <div className="flex gap-1.5">
                  {['20', '30', '50'].map((count) => (
                    <button
                      key={count}
        ...
        <div className="bg-neutral-900 border border-neutral-800 rounded-2xl overflow-hidden shadow-2xl">
          <div className="px-6 py-4 border-b border-neutral-800 flex justify-between items-center bg-neutral-900/50">
            <h2 className="text-base font-bold tracking-tight flex items-center gap-2"><span>✨</span> 검색 결과 </h2>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-neutral-800 bg-neutral-950/40 text-[11px] font-bold uppercase tracking-wider text-neutral-400">
                  ...
                  <th className="py-3 px-2 w-28 text-right">구독자수</th>
                ...
                {getProcessedResults().map((video) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 max-w-xs md:max-w-sm align-middle">
                      ...
                      <div className="text-neutral-400 font-medium text-[11px] flex items-center gap-1">
                ...
                {!isLoading && getProcessedResults().length === 0 && (
                  <tr>
                    <td colSpan={7} className="text-center py-16 text-neutral-500 font-medium">
                      {videos.length === 0 ? '검색어를 입력해 데이터를 조회해보세요.' : '필터 조건에 부합하는 영상이 없습니다.'}
                    </td>
                  </tr>
                )}
              </tbody>
